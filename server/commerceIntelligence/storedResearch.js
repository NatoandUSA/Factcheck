...
function normalizeXrayRows(rows = [], source = {}) {
  return rows.map((row, index) => ({
    ...
    rating: numberOrNull(lookup(row, XRAY.rating)), fulfillment: lookup(row, XRAY.fulfillment) ?? null,
    seller: lookup(row, XRAY.seller) ?? null, sellerCountry: lookup(row, XRAY.sellerCountry) ?? null,
    activeSellers: numberOrNull(lookup(row, XRAY.activeSellers)), category: lookup(row, XRAY.category) ?? null,
    creationDate: lookup(row, XRAY.creationDate) ?? null,
    url: lookup(row, XRAY.url) ?? null, imageUrl: lookup(row, XRAY.imageUrl) ?? null,
    ...
    buyBox: lookup(row, XRAY.buyBox) ?? null, sizeTier: lookup(row, XRAY.sizeTier) ?? null,
    dimensions: lookup(row, XRAY.dimensions) ?? null, weight: lookup(row, XRAY.weight) ?? null,
    abaMostClicked: lookup(row, XRAY.abaMostClicked) ?? null, sponsored: lookup(row, XRAY.sponsored) ?? null,
    bestSeller: lookup(row, XRAY.bestSeller) ?? null, sellerAgeMonths: numberOrNull(lookup(row, XRAY.sellerAgeMonths)),
    sourceFields: sourceFields(row),
