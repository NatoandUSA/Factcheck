// Browser-safe projection of the Product Truth contract. Server authority lives
// in productTruth.cjs plus the persisted attestation/audit binding.
export const EVIDENCE_STATES = Object.freeze({ VERIFIED: 'VERIFIED', UNKNOWN: 'UNKNOWN', UNVERIFIED: 'UNVERIFIED' });
export const IP_STATES = Object.freeze({ CLEARED: 'CLEARED', BLOCKED: 'BLOCKED', REVIEW_REQUIRED: 'REVIEW_REQUIRED', UNKNOWN: 'UNKNOWN' });
...
export function isEvidenceBoundToListing(evidence, context) {
...
export function isVerifiedFact(fact, context) {
  return Boolean(fact) && fact.value !== null && fact.value !== undefined && isEvidenceBoundToListing(fact.evidence, context);
}

export function getVerifiedPersonalization(card, context = card) {
  ...
  return instructions ? Object.freeze({ supported: true, instructions }) : null;
}

export function isVerifiedPersonalization(card, context = card) {
  return getVerifiedPersonalization(card, context) !== null;
}

export function isIpCleared(card, context = card) {
  const evidence = card?.ipEvidence;
  return isListingContext(context)
    && evidence?.state === IP_STATES.CLEARED
    && normalizeId(evidence.subjectId) === normalizeId(context.productId)
    && evidence.listingVersion === context.listingVersion
...
export function validateProductTruthCard(card, context = card) {
  const errors = [];
...
export function projectVerifiedFacts(card, context = card) {
...
export function invalidateProductTruthCard(card) {
