...
export default function ListingHistory({ history, onSelectListing, onDeleteListing, onClearHistory, onShowToast, onRefresh }) {
  ...
  const filteredHistory = history.filter(item => {
    ...
    return amzTitle.includes(query) || etsyTitle.includes(query) || cat.includes(query);
  });
    ...
    <div className="studio-panel" style={{ maxWidth: '1100px', margin: '0 auto', position: 'relative' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px', marginBottom: '20px' }}>
        <div>
          <h2 style={{ fontSize: '1.35rem', display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
            ...
            <span>Saved Product Catalog ({history.length})</span>
          </h2>
          <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
            Access and restore all your previously generated Amazon FBM & Etsy listings anytime.
          </p>
        </div>
        ...
        <div style={{ marginBottom: '18px' }}>
          <div style={{ position: 'relative' }}>
            ...
            <input
              ...
              placeholder="Search by keywords, category, or title..."
              value={searchTerm}
      ...
      {history.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '60px 20px', color: 'var(--text-secondary)' }}>
          ...
          <h4 style={{ fontSize: '1.1rem', marginBottom: '6px' }}>No saved listings yet</h4>
          <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>
            When you generate a listing, click "Save to Catalog" to keep it stored here.
          </p>
        </div>
      ) : filteredHistory.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '40px 20px', color: 'var(--text-muted)' }}>
          No listings match your search "{searchTerm}".
        </div>
      ) : (
