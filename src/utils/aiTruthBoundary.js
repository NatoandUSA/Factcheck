...
export function renderVerifiedCommerceListing(projection, modelOutput) {
  if (!isAuthorizedAiProjection(projection)) return null;
  ...
  return {
    ...
    amazonDescription: `<p>${profile.tone}</p><p>Product: ${escapeHtml(subject)}.${escapeHtml(materialSentence)}${escapeHtml(personalizationSentence)}</p>`,
...
export function validateModelClaims(output, projection) {
  ...
  const unexpectedFields = output && typeof output === 'object' ? Object.keys(output).filter(field => !SERVER_METADATA_FIELDS.has(field)) : [];
...
export const AI_FACTUAL_CLAIM_RULES = Object.freeze([]);
