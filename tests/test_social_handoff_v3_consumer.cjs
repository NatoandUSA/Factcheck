...
(async () => {
  ...
  try {
    ...
    const secondEnvelope = envelopeWithNonce('223e4567-e89b-42d3-a456-426614174000');
    await assert.rejects(() => pullAndPersistSocialHandoff({ db, scope, idempotencyKey: 'pull-social-0004', env,
      fetchImpl: async () => new Response(secondEnvelope, { status: 200,
    ...
    await assert.rejects(() => pullAndPersistSocialHandoff({ db, scope: { ...scope, role: 'MANAGER' },
      idempotencyKey: 'pull-social-0003', env, fetchImpl, now: NOW }), error => error?.code === 'SOCIAL_HANDOFF_OWNER_SCOPE_REQUIRED');
  } finally {
    await close(db);
  }
